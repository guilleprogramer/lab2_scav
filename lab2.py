...
def cut(start, duration, video, new_video):
    os.system('ffmpeg -ss '+start+' -i '+video+' -c copy -t '+duration+' '+new_video)
...
def mono(video,new_video):
    os.system('ffmpeg -i '+input+' -map_channel 0.1.0 -c:v copy '+new_video)
